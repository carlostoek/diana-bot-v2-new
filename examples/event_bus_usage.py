...
class GamificationHandler(BaseEventHandler):
    ...
    async def _process_event(self, event: IEvent) -> bool:
        ...
        if event.event_type == EventType.STORY_CHAPTER_STARTED:
            ...
            logger.info(f"Welcome bonus of 100 points for user {event.payload['user_id']}")
            return True
    ...
    async def _process_event(self, event: IEvent) -> bool:
        ...
        if event.event_type == EventType.POINTS_AWARDED:
            ...
            if event.payload["points_amount"] >= 100:
                logger.info(f"User {event.payload['user_id']} may have unlocked new story content")
...
class AnalyticsHandler(BaseEventHandler):
    ...
    async def _process_event(self, event: IEvent) -> bool:
        ...
        logger.info(f"Analytics tracking: {event.event_type} for user {event.payload.get('user_id', 'N/A')}")
...
async def demonstrate_event_handling(event_bus: RedisEventBus):
    ...
    sub1 = await event_bus.subscribe(EventType.STORY_CHAPTER_STARTED, gamification_handler)
