...
class TestBaseEventWithValidation:
    ...
    def test_custom_validation_hook(self):
        ...
        with pytest.raises(EventValidationError, match="Custom field must have required value"):
            TestEventWithCustomValidation(
                source_service="test",
                payload={"custom_field": "wrong_value"}
            )
        
        # Should pass custom validation
        event = TestEventWithCustomValidation(
            source_service="test",
            payload={"custom_field": "required_value"}
        )
        assert event.validate()
...
class TestDomainEvent:
    ...
    def test_domain_event_requires_user_id(self):
        ...
        with pytest.raises(EventValidationError, match="Domain events must have a user_id"):
