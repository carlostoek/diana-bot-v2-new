"""
Tests for the minimal implementation of PointsEngine
"""
...
class TestMinimalPointsEngine:
    """Test the minimal implementation of PointsEngine."""
    ...
    async def minimal_engine(self):
        ...
        return MinimalPointsEngine(anti_abuse_validator=validator)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_operations(self, minimal_engine):
        """Test concurrent operations maintain mathematical integrity."""
        user_id = 123
        num_operations = 20
        
        # Create concurrent award operations
        async def award_task(i):
            return await minimal_engine.award_points(
                user_id=user_id,
        ...
        assert total_balance == calculated_total, (
            f"Balance integrity violation after concurrent operations: "
            f"balance={total_balance}, transaction history={calculated_total}"
        )