...
# Try to import EventFactory from legacy events module first, then from new system
try:
    from .events import EventFactory
except ImportError:
    # Create a fallback EventFactory for new event system
    class EventFactory:
        @staticmethod
...
class RedisEventBus(IEventBus):
    ...
    async def shutdown(self) -> None:
        ...
        if self._processing_tasks:
            logger.info(
                ...
                self._processing_tasks, timeout=10.0, return_when=asyncio.ALL_COMPLETED
            )
    ...
    async def publish(
        ...
        try:
            ...
            raise EventPublishError(
                f"Failed to publish event: {str(e)}", event.event_id
            ) from e
    ...
    async def get_subscription_health(self) -> Dict[str, Dict[str, Any]]:
        ...
        for sub_id, subscription in self._subscriptions.items():
            health_data[sub_id] = {
                ...
                "last_processed": (
                    subscription.last_processed.isoformat()
                    if subscription.last_processed
                    else None
                ),
                "success_count": subscription.success_count,
                "error_count": subscription.error_count,
                "error_rate": subscription.error_count
                / max(1, subscription.success_count + subscription.error_count),
    ...
    async def _handle_event_with_retry(
        self, event: IEvent, subscription: Subscription
                ...
                if not await subscription.handler.can_handle(event):
                    return EventProcessingResult(
                        ...
                        error=EventHandlingError(
                            "Handler cannot process event", event.event_id
                        ),
                    )
...
class BaseEventHandler(IEventHandler):
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self._processing_count
            / max(1, self._processing_count + self._error_count),
            "average_processing_time_ms": avg_processing_time * 1000,
            "last_processed": (
                self._last_processed.isoformat() if self._last_processed else None
            ),
        }
