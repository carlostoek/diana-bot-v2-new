    ...
    def event_type(self) -> str:
        ...
        try:
            temp_instance = self.event_class(
                user_id=1, source_service=self.primary_publisher.value
            )
...
class EventCatalog:
    ...
    _GAMIFICATION_EVENTS = {
        PointsAwardedEvent: EventRoute(
            ...
            secondary_publishers={
                ServiceName.NARRATIVE
            },  # Can award points for story actions
        ),
    ...
    def get_route(
        self, event_class: Type[BaseEventWithValidation]
    ) -> Optional[EventRoute]:
        """
    ...
    def get_events_published_by(
        self, service: ServiceName
    ) -> List[Type[BaseEventWithValidation]]:
        """
    ...
    def get_events_by_category(
        ...
        for event_class in self._routes.keys():
            ...
            try:
                temp_instance = event_class(user_id=1, source_service="test")
...
def get_service_published_events(
    service: ServiceName,
) -> List[Type[BaseEventWithValidation]]:
...
def get_service_subscribed_events(
    service: ServiceName,
) -> List[Type[BaseEventWithValidation]]:
